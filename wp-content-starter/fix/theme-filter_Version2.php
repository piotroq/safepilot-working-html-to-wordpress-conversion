<?php
/**
 * SafePilot Theme Filter Functions
 * FIXED: Added null checks for screen objects
 * Created by PhpStorm.
 * User: Administrator
 * Date: 6/17/2016
 * Time: 10:32 AM
...
if (!function_exists('g5plus_body_class_name')) {
    function g5plus_body_class_name($classes)
    {
        ...
        if ($action === 'yith-woocommerce-view-table') {
...
/**
 * NAPRAWIONE: Editor Stylesheets z walidacją screen object
 */
if (!function_exists('g5plus_editor_stylesheets')) {
	function g5plus_editor_stylesheets($stylesheets) {
		$post_id = '';
		
		// NAPRAWIONE: Pełna walidacja screen object
		if (is_admin()) {
			$screen = get_current_screen();
			if (is_object($screen) && isset($screen->id) && $screen->id === 'post') {
				global $post;
