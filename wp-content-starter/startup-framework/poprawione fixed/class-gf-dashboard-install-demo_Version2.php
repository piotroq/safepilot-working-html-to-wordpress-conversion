...
if (!class_exists('GF_Dashboard_Install_Demo')) {
	class GF_Dashboard_Install_Demo {
		...
		public function  install_demo() {
			...
			// NAPRAWIONE: Zmiana nazwy klasy z G5_Import na WP_Import
			...
			if ($importer_error !== false) {
				...
				// NAPRAWIONE: Zmiana warunku z G5_Import na WP_Import
				if ( class_exists( 'WP_Import' ) ) {
					include_once( GF_PLUGIN_DIR . 'core/dashboard/install-demo/g5plus_import_class.php' );
				}

				// Sprawdzenie czy klasa GF_Import istnieje
				if ( ! class_exists( 'GF_Import' ) ) {
					ob_end_clean();
					$data_response = array(
						'code' => 'error',
						'message' => esc_html__("Import class not found. Please check g5plus_import_class.php file.",'startup-framework')
					);
					echo json_encode($data_response);
					die();
				}
				...
				switch (trim($type)) {
					case 'init':
						 ...
						 * Remove log file
						 */
						$log_dir = GF_PLUGIN_DIR . 'core' . DIRECTORY_SEPARATOR . 'dashboard' . DIRECTORY_SEPARATOR . 'install-demo' . DIRECTORY_SEPARATOR . "data" . DIRECTORY_SEPARATOR . "log";
						if ( is_dir( $log_dir ) && $handle = opendir( $log_dir ) ) {
							while ( false !== ( $entry = readdir( $handle ) ) ) {
								if ( $entry != "." && $entry != ".." ) {
									$file_path = $log_dir . DIRECTORY_SEPARATOR . $entry;
						...
						$sql_query = $wpdb->prepare("DELETE FROM $wpdb->postmeta WHERE 1 = %d", 1);
						$wpdb->query($sql_query);

						// posts
						$sql_query = $wpdb->prepare("DELETE FROM $wpdb->posts WHERE 1 = %d", 1);
						$wpdb->query($sql_query);
